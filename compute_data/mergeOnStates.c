...
void writeToFile(FILE *f, int race, int sex, int population, char buff[32])
{
  ...
  fprintf(f, "%c%c%c%c,%c%c,%c%c,%d,%d,%d%d%d%d%d%d%d%d%d\n", buff[0], buff[1], buff[2], buff[3], buff[4], buff[5], buff[6], buff[7],
...
int main()
{
  ...
  if ((fptr = fopen("../data/us_pop_merge_on_age_and_origin.txt","r")) == NULL){
   ...
   {
     current_state = (buff[7] - '0') + 10*(buff[6] - '0');
     int race = buff[11] - '0';
     int sex = buff[12] - '0';
     if (current_state - previous_state > 0)  //Another state.
     {
       writeToFile(wr, 1, 1, pop_11, previous_buff);
       writeToFile(wr, 1, 2, pop_12, previous_buff);
       writeToFile(wr, 2, 1, pop_21, previous_buff);
       writeToFile(wr, 2, 2, pop_22, previous_buff);
       writeToFile(wr, 3, 1, pop_31, previous_buff);
       writeToFile(wr, 3, 2, pop_32, previous_buff);
       writeToFile(wr, 4, 1, pop_41, previous_buff);
       writeToFile(wr, 4, 2, pop_42, previous_buff);
